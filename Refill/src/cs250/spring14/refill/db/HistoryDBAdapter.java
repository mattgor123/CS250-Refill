...
public class HistoryDBAdapter {
  ...
  public int removeHis(long ri) {
    open();
    return db.delete(H_TABLE, H_ID + " = ?", new String[] {String.valueOf(ri)});
  }

  /**
   * Method to get a cursor for all the HistoryItems Sorted in descending order for display purposes
   * 
   * @return the Cursor
   */
  ...
  private static class HisDBHelper extends SQLiteOpenHelper {
    ...
    private static final String DB_CREATE = "CREATE TABLE " + H_TABLE + " (" + H_ID
        + " INTEGER PRIMARY KEY AUTOINCREMENT," + H_OWN + " TEXT," + H_MSG + " TEXT, " + H_HT + " TEXT);";
    ...
    public void onUpgrade(SQLiteDatabase adb, int oldVersion, int newVersion) {
      // TODO Auto-generated method stub
      Log.w("Hisdb", "upgrading from version " + oldVersion + " to " + newVersion + ", destroying old data");
