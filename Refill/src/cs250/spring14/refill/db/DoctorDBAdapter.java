...
public class DoctorDBAdapter {
	...
	private static final int DB_VERSION = 14; // All Doctors must have unique
												// name
	 ...
	 * 
	 * @param ri
	 *            the row in the DB where this Doctor lives
	 * @param name
	 *            the Doctor's name
	 * @param email
	 *            the Doctor's email
	...
	public boolean updateDrByName(String dname, String name, String email,
		...
		if (dname != name && getDocByName(name) != null) {
			...
			cvalues.put(DR_EMAIL, email);
			cvalues.put(DR_PHONE, phone);
			return db.update(DR_TABLE, cvalues, DR_NAME + " = ?",
					new String[] { String.valueOf(dname) }) > 0;
		}
	}
	...
	private static class DrDBHelper extends SQLiteOpenHelper {
		...
		private static final String DB_CREATE = "CREATE TABLE " + DR_TABLE
				...
				+ DR_NAME + " TEXT UNIQUE," + DR_EMAIL + " TEXT," + DR_PHONE
				+ " TEXT);";

		public DrDBHelper(Context context, String name, CursorFactory fct,
				int version) {
