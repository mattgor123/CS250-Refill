...
public class PharmacyDBAdapter {
	...
	private static final int DB_VERSION = 16; // all pharmacies must have unique
												// name
	...
	public static final String[] PH_COLS = { PH_ID, PH_NAME, PH_EMAIL,
			PH_PHONE, PH_STR };
	 ...
	 * 
	 * @param ri
	 *            the row in the DB where this Pharmacy lives
	 * @param name
	 *            the Pharmacy's name
	 * @param email
	...
	public boolean updatePh(long ri, String name, String email, String phone,
		...
		return db.update(PH_TABLE, cvalues, PH_ID + " = ?",
	...
	public boolean updatePhByName(String pname, String name, String email,
		...
		if (pname != name && getPharmByName(name) != null) {
			...
			cvalues.put(PH_PHONE, phone);
			cvalues.put(PH_STR, street);
			return db.update(PH_TABLE, cvalues, PH_NAME + " = ?",
	...
	private static class PhDBHelper extends SQLiteOpenHelper {
		...
		private static final String DB_CREATE = "CREATE TABLE " + PH_TABLE
				...
				+ PH_NAME + " TEXT UNIQUE," + PH_EMAIL + " TEXT," + PH_PHONE
				+ " TEXT, " + PH_STR + " TEXT);";

		public PhDBHelper(Context context, String name, CursorFactory fct,
				int version) {
